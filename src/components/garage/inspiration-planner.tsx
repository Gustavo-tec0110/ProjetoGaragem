...
export function InspirationPlanner({
  ...
  const normalizedPlannerState = React.useMemo(() => {
    ...
    const validReference =
      ...
      referencePool[0]?.slug ??
      inspirationOptions[0]?.slug ??
      "";
  ...
  }, [normalizedPlannerState, storageScope]);
  ...
  const analysis = React.useMemo(
    () => analyzeProjectInspiration(currentProject, referenceProject),
    ...
    <Card className="p-5 md:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-2 text-sm text-muted">
            Esta porcentagem mostra o quanto o estado atual do seu carro se aproxima da
            build escolhida como referencia. Nao representa conclusao do projeto.
          </p>
        </div>
        ...
        <label className="grid gap-2 text-sm text-muted">
          ...
          >
            {inspirationOptions
              .filter(
                (project) =>
                  project.slug !== normalizedPlannerState.currentSlug ||
                  inspirationOptions.length === 1
              )
              .map((project) => (
                <option key={project.slug} value={project.slug}>
                  {project.title}
